...
|---|---|---|---|
| Ưu tiên cao | Đỏ | `application.priority = HIGH` + có interview | Ứng viên cần phỏng vấn gấp |
| Đã xác nhận | Xanh lá | `interview.status = CONFIRMED` | Ứng viên đã xác nhận tham dự |
| Đang xử lý | Xanh dương | `interview.status = SCHEDULED` | Chờ ứng viên xác nhận |
| Chờ phản hồi | Vàng amber | `interview.status = PENDING_RESCHEDULE` | Ứng viên đang đề xuất đổi lịch |
...
Validate toàn bộ form
    ...
    └─ Hợp lệ → Gọi API
                ...
                Badge "Đang xử lý" +1 (sidebar phân loại)
```
...
| Ngày | Bắt buộc, không được là ngày quá khứ |
| Giờ bắt đầu | Bắt buộc, phải cách hiện tại ≥ 1 giờ (nếu là hôm nay) |
...
| Ứng viên confirm lịch | Badge đổi từ "Đang xử lý" → "Đã xác nhận" |
| Ứng viên đề xuất đổi lịch | Badge đổi → "Chờ phản hồi", toast thông báo HR |
...
| Click vào ngày trong quá khứ | Modal mở nhưng field ngày báo lỗi ngay, không cho submit |
